...
CONFIG_PATH = os.path.join(ROOT, "config_llama.yaml")

# --- Muat Konfigurasi dari YAML (Jika Ada) ---
...
def clean_user_query(query: str) -> str:
    pattern = r'^(carikan|tolong carikan|bantu cari|tunjukkan|cari tentang|bisa carikan|tolong tampilkan|apakah ada)\s+'
...
def search_tool(
    ...
    if embedding_model is not None and chroma_collection is not None:
        try:
            # --- TAHAP 1: MENGUKUR LATENSI VECTOR SEARCH ---
            ...
            # [PERUBAHAN 2] Petakan teks utuh (documents) berdasarkan doc_id
            full_docs_map = {}
            ...
            try:
                # --- TAHAP 2: MENGUKUR LATENSI RERANKING ---
                ...
                for doc in final_ranked:
                    ...
                    # [PERUBAHAN 3] Tarik teks FULL dari mapping, bukan metadata
                ...
                if db_results['metadatas'] and db_results['metadatas'][0]:
                    for i, meta in enumerate(db_results['metadatas'][0][:top_k]):
                        ...
                        # [PERUBAHAN 4] Ambil teks utuh di fallback
...
[ATURAN KETAT]
1. Pencocokan Semantik: Anda diizinkan dan WAJIB memahami makna pertanyaan untuk dicocokkan dengan sinonim atau padanan kata di dalam konteks (misal: "Syarat" setara dengan "Kualifikasi", "SK" setara dengan "Keputusan", "Tujuan" setara dengan "Guna"). Jangan menolak menjawab hanya karena perbedaan istilah bahasa.
2. Penalaran Temporal: Jika pertanyaan menanyakan data "terbaru", "saat ini", atau status aktif, perhatikan angka tahun pada dokumen dan gunakan logika deduktif untuk memilih periode yang paling mutakhir.
3. Susun jawaban dalam kalimat lengkap yang merangkum kembali konteks pertanyaan pengguna (Full Sentence). Jangan menjawab hanya dengan satu frasa atau nama saja.
4. Fallback: Jika, dan hanya jika setelah melakukan pencocokan makna dan logika di atas, informasi masih benar-benar tidak ada di dalam [KONTEKS ARSIP], Anda WAJIB menjawab persis dengan kalimat: "Maaf, informasi tersebut tidak ditemukan di dalam dokumen."
5. JANGAN mengarang informasi, angka, tanggal, atau nomor SK (Dilarang keras berhalusinasi).
6. JANGAN menggunakan frasa pengantar seperti "Berdasarkan dokumen referensi..." atau menyebutkan metadata dokumen. Langsung berikan inti jawabannya secara natural.
7. Jika konteks memuat prosedur, sebutkan langkah-langkahnya secara berurutan.
8. Jika konteks memuat nomor SK atau data spesifik, sebutkan secara eksplisit HANYA jika relevan dengan pertanyaan.
