...
OUTPUT_NAME = "minilm-dokumen-arsip-boosted-seranah-seed-42"
...
def load_augmented_data(filename="train_seranah.jsonl"):
        ...
        for line in f:
            try:
                ...
                if "title" in data and "content" in data:
                    ...
                    # 1. Pasangan Positif: Title & Content
                    examples.append(InputExample(texts=[title, content]))
                    count_title_content += 1

                    # 2. Pasangan Positif: Snippet Content & Title
                    content_snippet = content[:200]
                    if len(content_snippet) > 50:
                        examples.append(InputExample(texts=[content_snippet, title]))
                        count_snippet_title += 1

                    # 3. Pasangan Positif: Keywords & Title (diambil langsung dari field 'keywords')
                    keywords = data.get("keywords", "").strip()
                    # Fallback jika ada format lama yang menyisipkan 'kata kunci' di content
    ...
    print(f"    - Keywords dengan Title             : {count_keyword_title} pasang")

    return examples


def train_boosted():
    # 1. Load Data Augmented dari dataset train_seranah.jsonl
    train_examples_list = load_augmented_data("train_seranah.jsonl")
