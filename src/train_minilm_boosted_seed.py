...
OUTPUT_NAME = "minilm-dokumen-arsip-boosted-new"
...
def train_boosted():
    ...
    print(f"\n[INFO] Memulai Eksperimen dengan {len(SEEDS)} Seed Berbeda: {SEEDS}")

    for seed in SEEDS:
        print(f"\n{'=' * 60}")
        print(f"🚀 MULA TRAINING UNTUK SEED: {seed}")
        print(f"{'=' * 60}")
        ...
        print(f"[*] Menyimpan model final ke {current_output_dir}...")
        ...
        # Simpan Riwayat Loss
        loss_path = os.path.join(current_output_dir, "loss_history.json")
        ...
        print(f"[*] Data riwayat Loss dan stats tersimpan di: {current_output_dir}\n")

    print(f"\n{'=' * 60}")
    print(f"🎉 SEMUA {len(SEEDS)} EKSPERIMEN SELESAI!")
    print(f"Model-model tersimpan di dalam folder: {BASE_OUTPUT_DIR}")
    print(f"{'=' * 60}")
