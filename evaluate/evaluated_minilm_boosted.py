...
def run_evaluation():
    ...
    if not os.path.exists(MODEL_PATH):
        print(f"[!] Folder model tidak ditemukan. Jalankan train_minilm_boosted.py dulu!")
        return
    ...
    # 4. Jalankan Evaluasi
    print("[*] Memulai evaluasi MiniLM BOOSTED...")
    results = evaluator(model)

    # 5. Tampilkan Hasil
    print("\n" + "=" * 40)
    print(f" HASIL EVALUASI MINILM (BOOSTED)")
    print("=" * 40)
    ...
    # Simpan hasil
    output_json = os.path.join(ROOT, "output", "evaluation_minilm_boosted_results_testing_new_datatest_fix.json")
